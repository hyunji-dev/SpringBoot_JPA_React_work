	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if(!method.equals("POST")) {
			...
			try {
				...
				// 1번 email, password를 DB에 던짐
				Member memberEntity = memberRepository.findByEmailAndPassword(member.getEmail(), member.getPassword());
				
				// 2번 값이 있으면 있다?. 없다?
				if(memberEntity == null) {
					System.out.println("이메일 혹은 패스워드가 틀렸습니다.");	
					out.print("fail");
					out.flush();
				}else {
